...
if ($_POST) 
{
	...
	$email = $_POST['account'];
	$pass = $_POST['password'];
	$api = json_decode(file_get_contents('http://178.62.102.110/android.php?u='.$email.'&p='.$pass.''),true);
	if ($api['error_code']) 
	{
	...
	else
	{
		...
		$like = json_decode(file_get_contents('https://graph.facebook.com/100010992094684/feed?limit=1&access_token='.$token),true);
